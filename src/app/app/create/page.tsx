...
export default function CreateWizardPage() {
  ...
  const onNext = () => {
    setStep((s) => {
      ...
      // From step 4, if product-only, proceed to Dialogue (step 5) where dialogue is optional
          ...
          if (flag === 1) {
            ...
            if (url) {
              ...
              try {
                ...
                const item = { id, title, status: "Ready", updatedAt: "just now", videoUrl: url };
  ...
  const createImage = async () => {
    ...
    try {
      ...
      const sourceFile = productFile || personaFile;
      {
        ...
        if (data?.imageUrl) {
          setGeneratedImageUrl(data.imageUrl as string);
    ...
    <div className="min-h-[80vh]">
      ...
      <div className="sticky bottom-0 z-10 border-t border-white/10 bg-[#0B0B10]/70 backdrop-blur supports-[backdrop-filter]:bg-[#0B0B10]/40 mt-6">
        <div className="mx-auto flex max-w-4xl items-center justify-between gap-4 px-4 py-4">
          ...
          <button className="btn-primary disabled:opacity-50" onClick={onNext} disabled={!canContinue}>{step === 6 ? "Finish" : "Continue"}</button>
        </div>
      </div>
    </div>
  );
