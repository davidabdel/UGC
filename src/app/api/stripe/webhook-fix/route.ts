  ...
  try {
    ...
    switch (event.type) {
      ...
      case 'checkout.session.completed': {
        ...
        // For testing, we'll use the checkout session data directly instead of retrieving from Stripe
        try {
          // Get the price ID from the line items or use a direct lookup
          // In a real checkout session, we'd need to use the Stripe API to get this
          // But for testing, we can use the price ID from the form
          
          // This is a mock subscription object for testing
          ...
          // For testing, we'll extract the price ID from the URL parameters
          const url = new URL(req.url);
          const priceId = url.searchParams.get('price_id') || 'price_1SELv9KIeF7PCY4J8o8mvjHB'; // Default to Business Monthly
          ...
          const result = await upsertSubscription(
            ...
            mockSubscription.id,
            status,
            new Date(mockSubscription.current_period_start * 1000).toISOString(),
            new Date(mockSubscription.current_period_end * 1000).toISOString(),
